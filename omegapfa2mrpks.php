...
function omegapfa2mrpks_generate() {
    ...
    $txt_prijate_fa = $target_dir . $sess_id . '_' . $_FILES['f_txt']["name"];
    ...
    if (file_exists($txt_prijate_fa)) {

      if (($handle = fopen($txt_prijate_fa, "r")) !== FALSE) {
        stream_filter_append($handle, 'convert.iconv.windows-1250.utf-8');

        //hlavicka suboru pre import
        $export_data = '<?xml version="1.0" encoding="UTF-8"?><MRPKSData version="2.0" countryCode="SK" currencyCode="EUR"><IncomingInvoices>';
                      ...
                      switch($row_data[0]){
                        ...
                        case "R02":
                              ...
                              */
                              ...
                              $data = create_polozka($row_data,$person_type, $vat_code);
                              if($data['result'] == -1){
            ...
            $export_data .= '</IncomingInvoices></MRPKSData>';
...
function create_faktura($row_data){
  ...
  $xml_data = '<Invoice>';
  $data['fa_cislo'] = substr($row_data[43],0,20);
...
//overit ci je to c100
  ...
  if(substr($row_data[39],0,5) != "EUR"){
    $xml_data .= '<CurrRate>'.substr($row_data[40],0,10).'</CurrRate>';
    $xml_data .= '<CurrRateAmount>'.substr($row_data[41],0,20).'</CurrRateAmount>';
  }
...
//Company
  ...
  $xml_data .= '</Company>';
  $vat_code = substr(trim($row_data[47]),0,2);

  if ($vat_code == "SK"){
    $xml_data .= '<TaxCode>40</TaxCode>';
    $data['tax_code_set']=1;
  }else if ($person_type == 'T' && $vat_code == ""){
    $xml_data .= '<TaxCode>39</TaxCode>';
    $data['tax_code_set']=1;     
  } else if ($person_type == 'F'){
    $xml_data .= '<TaxCode>49</TaxCode>';
    $data['tax_code_set']=1;     
  }
...
function create_polozka($row_data,$person_type, $vat_code){
  $xml_data = "";
  $xml_data .= '<Item>';
...
/*
  ...
  */
  ...
  $xml_data .= '<UnitPrice>'.number_format($row_data[7], 6, '.', '').'</UnitPrice>';
  ...
  if ($vat_code == "SK" || $person_type == 'T'){
    ...
    $xml_data .= '<TaxPercent>20</TaxPercent>';  
    $xml_data .= '<TaxAmount>'.number_format(($row_data[7]*0.2), 6, '.', '').'</TaxAmount>';
    }
